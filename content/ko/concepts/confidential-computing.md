...
기밀 컴퓨팅(confidential computing)은 하드웨어 기반의 보호된 환경으로 평문(plaintext) 노출을 제한함으로써 연산 중인 데이터를 보호하는 광범위한 패턴이다. 신뢰 실행 환경(TEE, Trusted Execution Environment)이 가장 일반적인 구현 방식이다. 출처: [FPF 기밀 컴퓨팅과 프라이버시](../../ko/sources/background-2025-04-confidential-computing-and-privacy.md), [Confidential Computing Consortium 참조](../../ko/sources/background-2024-confidential-computing-consortium.md).
...
Apple PCC는 기밀 컴퓨팅에 인접하지만, 일반적인 TEE 이야기로 축소해서는 안 된다. Apple의 공개적 강조점은 디바이스 기반 신뢰(device-rooted trust), 공개 릴리스 투명성, 표적 요청 보호(targeted-request protections)에 있으며, 단순한 엔클레이브(enclave) 기밀성에만 국한되지 않는다. 출처: [Apple PCC 소개](../../ko/sources/apple-2024-06-10-private-cloud-compute-intro.md), [Apple PCC 보안 연구](../../ko/sources/apple-2024-10-24-pcc-security-research.md).
...
- [Confidential Computing Consortium 참조](../../ko/sources/background-2024-confidential-computing-consortium.md)
