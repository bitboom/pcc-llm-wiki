...
Apple은 PCC가 RSA 블라인드 서명(RSA blind signatures) 기반의 일회용 자격 증명(single-use credential)을 사용하여, 특정 사용자와 연결하지 않으면서 유효한 요청을 인가한다고 명시하고 있다. 이는 Apple 공개 자료 중 라우팅 프라이버시(routing privacy)에 관한 가장 구체적인 세부 사항 중 하나이다. 출처: [Apple PCC 소개](../../ko/sources/apple-2024-06-10-private-cloud-compute-intro.md).
